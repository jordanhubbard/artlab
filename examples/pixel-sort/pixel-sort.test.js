...
describe('pixel-sort', () => {
  ...
  it('setup() creates _texture and _pixelData', () => {
    mod.setup(ctx)
    expect(ctx._texture).toBeInstanceOf(Three.DataTexture)
    expect(ctx._pixelData).toBeInstanceOf(Uint8Array)
  })

  it('setup() creates _quad mesh', () => {
    mod.setup(ctx)
    expect(ctx._quad).toBeInstanceOf(Three.Mesh)
  })

  it('setup() initializes sort state', () => {
    mod.setup(ctx)
    expect(ctx._sortRow).toBe(0)
    expect(ctx._time).toBe(0)
  })
