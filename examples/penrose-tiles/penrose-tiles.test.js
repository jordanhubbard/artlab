...
describe('penrose-tiles', () => {
  ...
  it('setup() creates _triangles array', () => {
    mod.setup(ctx)
    expect(Array.isArray(ctx._triangles)).toBe(true)
    expect(ctx._triangles.length).toBeGreaterThan(0)
  })

  it('setup() creates _tileMeshes array', () => {
    mod.setup(ctx)
    expect(Array.isArray(ctx._tileMeshes)).toBe(true)
    expect(ctx._tileMeshes.length).toBeGreaterThan(0)
  })

  it('setup() initializes _animTime', () => {
    mod.setup(ctx)
    expect(ctx._animTime).toBe(0)
  })
